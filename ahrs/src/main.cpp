/* MPU9250_MS5637_t3 Basic Example Code
 ...
 We have disabled the internal pull-ups used by the Wire library in the WireObj.h/twi.c utility file.
...
#define USE_I2C1 true

#ifdef USE_I2C1
i2c_t3 WireObj = i2c_t3(1);
#else
i2c_t3 WireObj = i2c_t3(0);
#endif
...
{
  WireObj.beginTransmission(address);  // Initialize the Tx buffer
  WireObj.write(subAddress);           // Put slave register address in Tx buffer
  WireObj.write(data);                 // Put data in Tx buffer
  WireObj.endTransmission();           // Send the Tx buffer
}
...
uint8_t readByte(uint8_t address, uint8_t subAddress)
{
  ...
  WireObj.beginTransmission(address);         // Initialize the Tx buffer        
  WireObj.write(subAddress);                  // Put slave register address in Tx buffer
  WireObj.endTransmission(I2C_NOSTOP);        // Send the Tx buffer, but send a restart to keep connection alive
  WireObj.requestFrom(address, (size_t) 1);   // Read one byte from slave register address 
  data = WireObj.read();                      // Fill Rx buffer with result
...
{  
  ...
  WireObj.endTransmission(I2C_NOSTOP);  // Send the Tx buffer, but send a restart to keep connection alive
  uint8_t i = 0;
  WireObj.requestFrom(address, (size_t) count);  // Read bytes from slave register address 
  while (WireObj.available()) {
    dest[i++] = WireObj.read(); // Put read results in the Rx buffer
  } 
}
...
// MS5637 registers
        ...
        void MS5637Reset()
        {
        WireObj.beginTransmission(MS5637_ADDRESS);  // Initialize the Tx buffer
  WireObj.write(MS5637_RESET);                // Put reset command in Tx buffer
  WireObj.endTransmission();                  // Send the Tx buffer
        }
        ...
        for (uint8_t ii = 0; ii < 7; ii++) {
          WireObj.beginTransmission(MS5637_ADDRESS);  // Initialize the Tx buffer
          WireObj.write(0xA0 | ii << 1);              // Put PROM address in Tx buffer
          WireObj.endTransmission(I2C_NOSTOP);        // Send the Tx buffer, but send a restart to keep connection alive
    uint8_t i = 0;
          WireObj.requestFrom(MS5637_ADDRESS, 2);   // Read two bytes from slave PROM address 
    while (WireObj.available()) {
          data[i++] = WireObj.read(); }               // Put read results in the Rx buffer
        ...
        WireObj.beginTransmission(MS5637_ADDRESS);  // Initialize the Tx buffer
        WireObj.write(CMD | OSR);                  // Put pressure conversion command in Tx buffer
        WireObj.endTransmission(I2C_NOSTOP);        // Send the Tx buffer, but send a restart to keep connection alive
        ...
        WireObj.beginTransmission(MS5637_ADDRESS);  // Initialize the Tx buffer
        WireObj.write(0x00);                        // Put ADC read command in Tx buffer
        WireObj.endTransmission(I2C_NOSTOP);        // Send the Tx buffer, but send a restart to keep connection alive
  uint8_t i = 0;
        WireObj.requestFrom(MS5637_ADDRESS, 3);     // Read three bytes from slave PROM address 
  while (WireObj.available()) {
        data[i++] = WireObj.read(); }               // Put read results in the Rx buffer
...
#ifdef USE_I2C1
  WireObj.begin(I2C_MASTER, 0x00, I2C_PINS_29_30, I2C_PULLUP_EXT, I2C_RATE_400);
#else
  WireObj.begin(I2C_MASTER, 0x00, I2C_PINS_16_17, I2C_PULLUP_EXT, I2C_RATE_400);
#endif
  delay(2000);
...
// Second order corrections
 ...
 //
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
    ...
    Serial.print((float)Temperature, 2); Serial.println("");
    /*
